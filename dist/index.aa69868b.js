...
function showSlide(sl) {
    ...
    console.log(slides);
    if (sl > slides.length) slideIndex = 1;
    if (sl < 1) slideIndex = slides.length;
    for(i = 0; i < icons.length; i++)slides[i].style.display = "none";
    for(i = 0; i < icons.length; i++)icons[i].className = icons[i].className.replace(" active", "");
